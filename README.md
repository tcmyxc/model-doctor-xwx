...
- 挑选卷积核（使用对应类别图片的loss更新对应的卷积核，未加说明th=0.5）
    - lr=0.01
        ...
        - 同上，但是反向调整卷积核: 66.06%
        - 自定义学习率，类别平衡采样(CBS)，REFL，不更新卷积核: **77.20%**
    - lr=1e-4
        - 自定义学习率，所有类别，所有层: 74.14%
        - 同上，但是反向调整卷积核: 74.16%
        - 余弦退火，其余同1: 74.81%
    - lr=1e-5
        - 自定义学习率，所有类别，所有层: **75.77%**
        - 余弦退火，其余同1: 75.33%
        - 自定义学习率，所有类别，所有层，th=0.4: 74.11%
        - 自定义学习率，所有类别，所有层，th=0.6: 75.05%
        - 使用类别均衡采样策略，其余同1: 75.55%(中途服务器挂了，只跑了165轮)
    - lr=1e-6
        - 自定义学习率，所有类别，所有层: **75.90%**
        - 自定义学习率, 所有类别, 所有层, th=0.4: 75.57%
        - 使用类别均衡采样策略，其余同1: todo
...
6. 不调整卷积核，仅仅在预训练模型上面加了CBS和REFL，不调整卷积核，结果比以前都要好（好过调整卷积核的实验）
