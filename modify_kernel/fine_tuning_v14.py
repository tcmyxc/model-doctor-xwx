...
def main():
    ...
    # grad mask
    grad_mask_path = "/nfs/xwx/model-doctor-xwx/output/result/resnet32-cifar-10-lt-ir100/stage3/grads/grad_mask/"
    for layer in model_layers:
        mask_path_patten = f"{grad_mask_path}/grad_mask_layer_{layer}.npy"
        modify_dict = np.load(mask_path_patten, allow_pickle=True)
    ...
    for epoch in range(epochs):
        ...
        train(data_loaders["train"], model, loss_fn, optimizer, modules, model_layers, device)
...
def train(dataloader, model, loss_fn, optimizer, modules, model_layers, device):
    ...
    for batch, (X, y, _) in enumerate(dataloader):
        ...
        with torch.set_grad_enabled(True):
            ...
            for layer in model_layers:
                if layer <= 19:
                    continue
                kernel_length = len(modify_dicts[layer])
                for kernel_index in range(kernel_length):
                    kernel_index = int(kernel_index)
                    # print(kernel_index)
                    if modify_dicts[layer][kernel_index] >= 2:
