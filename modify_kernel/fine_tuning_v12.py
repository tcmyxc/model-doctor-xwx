...
def main():
    ...
    for epoch in range(epochs):
        ...
        train(data_loaders["train"], model, loss_fn, optimizer, modules, 1-epoch/epochs, device, cfg)
...
def train(dataloader, model, loss_fn, optimizer, modules, epoch_decay, device, cfg):
    ...
    for batch, (X, y, _) in enumerate(dataloader):
        ...
        for cls, modify_dict in enumerate(modify_dicts):
            with torch.set_grad_enabled(True):
                ...
                pred, feature_out = model(x_cls_i)  # 网络前向计算
                loss = loss_fn(pred, y_cls_i)
                ...
                for kernel_index in range(modify_dict[layer][0]):
                    if cls >= 8 and kernel_index not in kernel_tail:
